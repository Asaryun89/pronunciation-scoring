...
_DIM_NAMES: List[str] = ["total", "accuracy", "fluency", "prosodic"]


def _pearson_r_1d(pred: Tensor, target: Tensor) -> Tensor:
    """Pearson r for a single pair of [B] vectors. Returns a scalar."""
    vp  = pred   - pred.mean()
...
def pcc_loss(pred: Tensor, target: Tensor) -> Tensor:
    """
    (1 − Pearson r) loss.  Accepts either [B] (single dim) or [B, D] (multi-dim).
    When multi-dim, averages over dimensions.
    """
    if pred.dim() == 1:
...
def compute_loss(
    ...
    """
    Compute per-dimension MSE + PCC loss, then average over dimensions.

    Args:
        pred, target: [B, 4] — normalised scores in [0, 1]

    Returns:
        total_loss: scalar Tensor for backprop
        dim_losses: {"loss_total": ..., "loss_accuracy": ..., ...}
    """
    losses: List[Tensor] = []
    ...
    for i, name in enumerate(_DIM_NAMES):
        p = pred[:, i]
        t = target[:, i]
        d_loss = mse_weight * F.mse_loss(p, t) + pcc_weight * pcc_loss(p, t)
...
_STEP_COLS  = [
    "step", "epoch", "loss",
    "loss_total", "loss_accuracy", "loss_fluency", "loss_prosodic",
...
class FusionCTrainer:
    ...
    def _step(self, batch: Dict) -> Dict[str, float]:
        ...
        with torch.autocast(device_type=self.device.type, enabled=use_amp):
            ...
            loss, dim_losses = compute_loss(pred, targets, self.mse_w, self.pcc_w)
        ...
        return {"loss": loss.item(), "grad_norm": grad_norm, "lr": self._current_lr(),
                **dim_losses}
    ...
    def _train_epoch(self, epoch: int) -> float:
        ...
        for batch in self.train_loader:
            ...
            if self.global_step % self.tcfg.get("log_every", 20) == 0:
                dim_str = "  ".join(
                    f"{n[:3]}={metrics[f'loss_{n}']:.3f}" for n in _DIM_NAMES
                )
                log.info(
                    "[Ep %d | Step %d] loss=%.4f  %s  lr=%.2e gnorm=%.2f | %s",
                    epoch, self.global_step, metrics["loss"], dim_str,
                ...
                self._step_csv.write({
                    ...
                    **{k: _fmt(metrics[k]) for k in metrics if k.startswith("loss_")},
    ...
    def _validate(self) -> Dict[str, float]:
        ...
        preds   = np.concatenate(all_pred,   axis=0)   # [N, 4]
        targets = np.concatenate(all_target, axis=0)   # [N, 4]
